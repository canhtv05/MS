...
type TabsHighlightItemProps<T extends React.ElementType = 'div'> = HighlightItemProps<T> & {
  value: string;
};

function TabsHighlightItem<T extends React.ElementType = 'div'>({
  value,
  ...props
}: TabsHighlightItemProps<T>) {
  return (
    <HighlightItem
      data-slot="tabs-highlight-item"
      value={value}
      {...(props as HighlightItemProps<T>)}
    />
  );
}
